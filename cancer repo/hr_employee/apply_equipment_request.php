...
<?php
	if(isset($_POST['apply'])) {
		...
		foreach ($equipment_categories as $key => $equipment_category) {
			...
			$sql = "INSERT INTO tblequipmentrequest (emp_id, equipment_category, quantity, reason, postingDate, adminRemark, adminRemarkDate, status, file_name, file_data) 
								...
								<div class="row">
									<div class="col-md-6 col-sm-12">
										<div class="form-group">
											...
											<input name="emp_id" type="text" class="form-control wizard-required" required="true" readonly autocomplete="off" value="<?php echo $row['emp_id']; ?>">
										</div>
									</div>
									<div class="col-md-6 col-sm-12">
										<div class="form-group">
											<label >Name</label>
											<input name="name" type="text" class="form-control" readonly required="true" autocomplete="off" value="<?php echo $row['FirstName']." ".$row['LastName']; ?>">
										</div>
									</div>
								</div>
								<div class="row">
									...
									<div class="col-md-6 col-sm-12">
										<div class="form-group">
											<label >Email</label>
											<input name="email" type="text" class="form-control" readonly required="true" autocomplete="off" value="<?php echo $row['EmailId']; ?>">
										</div>
									</div>
								</div>
        ...
        <div class="row">
            <div class="col-md-6 col-sm-12">
                <div class="form-group">
                    ...
                    <select name="equipment_category[]" class="custom-select form-control" required="true" autocomplete="off">
					<option value="">Select Equipment type...</option>
											<?php $sql = "SELECT  type_name from tblequipment_types";
											$query = $dbh -> prepare($sql);
											$query->execute();
											...
											{   ?>                                            
											<option value="<?php echo htmlentities($result->type_name);?>"><?php echo htmlentities($result->type_name);?></option>
											<?php }} ?>
                    </select>
                </div>
            </div>
									...
									<div class="col-md-4 col-sm-12">
										<div class="form-group">
											...
											<div class="modal-footer justify-content-center">
												<button class="btn btn-primary" name="apply" id="apply" data-toggle="modal">Submit&nbsp;Request</button>
											</div>
										</div>
									</div>
								</div>
