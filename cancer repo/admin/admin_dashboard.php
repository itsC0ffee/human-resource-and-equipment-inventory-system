...
<body>
    ...
    <div class="main-container">
                    ...
                    <div class="card-box height-100-p widget-style3">
                        ...
                        <div class="d-flex flex-wrap">
                            ...
                            <div class="widget-icon">
                                <div class="icon" data-color="#00eccf"><img src="../vendors/images/total_emp_icon.svg"
                                        alt=""></div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-xl-3 col-lg-3 col-md-6 mb-20">
                    <div class="card-box height-100-p widget-style3">
                        <?php
							...
							// Second Code
							$sql = "SELECT COUNT(emp_id) AS empcount FROM hr_employees";
							$query = $dbh->prepare($sql);
							$query->execute();
							$result = $query->fetch(PDO::FETCH_ASSOC);
							$empcount = $result['empcount'];

							// Third Code
							$currentDate = date("Y-m-d");
							$sql = "SELECT COUNT(id) AS count_leaves
									FROM hr_leaves
									WHERE :currentDate BETWEEN CAST(from_date AS DATE) AND CAST(to_date AS DATE)";
							$query = $dbh->prepare($sql);
							$query->bindParam(':currentDate', $currentDate);
							$query->execute();
							$result = $query->fetch(PDO::FETCH_ASSOC);
		...
		$sql = "SELECT COUNT(id) AS count_leaves
				FROM hr_leaves
				WHERE CAST(:currentDate AS DATE) BETWEEN from_date::DATE AND to_date::DATE";
		$query = $dbh->prepare($sql);
		$query->bindParam(':currentDate', $currentDate);
		$query->execute();
		$result = $query->fetch(PDO::FETCH_ASSOC);
