...
<?php
	...
	{ 
		...
		if ($status === '2') {
			$sql="update tblleaves set registra_remarks=:description,admin_status=:status where id=:did";
		...
		elseif ($status === '1') {
				$result = mysqli_query($conn,"update tblleaves, tblemployees set tblleaves.registra_remarks='$description',tblleaves.admin_status='$status',tblleaves.Status='$status', tblemployees.Av_leave='$REMLEAVE' where tblleaves.empid = tblemployees.emp_id AND tblleaves.id='$did'");
